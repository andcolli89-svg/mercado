...
const { productFromUrl, extractPriceCandidates, moneyFromAriaLabel } = require('../src/services/productService');
...
test('health informa versão e recursos da V6.3', async () => {
  await withServer(async port => {
    const response = await fetch(`http://127.0.0.1:${port}/health`);
    assert.equal(response.status, 200);
    const data = await response.json();
    assert.equal(data.version, '6.3.0');
...
test('Radar preserva preço promocional e original da cadeira real', () => {
  const item = normalizeRadarItem({
    title: 'Cadeira de Escritório Presidente Ergonômica Python Fly',
    price: '475,96',
    oldPrice: '1.248,75',
    link: 'https://produto.mercadolivre.com.br/MLB-123456789'
  });
...
test('aria-label do Mercado Livre preserva reais e centavos', () => {
...
test('extrator escolhe preço promocional e ignora parcela e cashback', () => {
  const html = '<div class="ui-pdp-price__main-container">' +
    '<span class="andes-money-amount ui-pdp-price__original-value" aria-label="1.248 reais com 75 centavos"></span>' +
    '<span class="andes-money-amount" aria-label="475 reais com 96 centavos"></span>' +
    '<span aria-label="46 reais com 3 centavos"></span>' +
    '</div>';
  const text = 'Oferta do dia R$ 1.248,75 R$ 475,96 61% OFF 12x R$ 46,03 R$ 728 de cashback';
  const result = extractPriceCandidates(html, text, '1248,75');
